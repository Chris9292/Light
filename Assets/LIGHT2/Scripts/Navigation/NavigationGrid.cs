...
public class NavigationGrid : MonoBehaviour
{
    ...
    public List<Node> GetNeighboringNodes(Node a_NeighborNode)
    {
        ...
        //Check for neighbors (8 total neightbors)
        for(int x = -1; x <= 1; x++) {
            for (int y = -1; y <= 1; y++) {
    ...
    public void createDynamicPathObjects()
    {
        if (FinalPath != null)//If the final path is not empty
        {

            pathRenderer.positionCount = FinalPath.Count + 1; // +1 for the starting position = player position
            ...
            foreach (Node n in FinalPath)
            {
                    ...
                    if (pooledObject)
                    {
                        ...
                        pathRenderer.SetPosition(count, n.vPosition);
                        count++;
                    }
            }
        }
